...
For more complex relationships you can pass blocks for calculated values, or related values. Following the previous example here is a comma set using blocks (both with and without labels for your CSV headings):
 ...
 class Book < ActiveRecord::Base
   ...
   # ===============
   comma do
     publishers_contact     = lambda { |publisher| publisher.primary_contact.name.to_s.titleize }
     total_publisher_users  = lambda { |publisher| publisher.users.size }
     name
     description

     pages :size => 'Pages'
     publisher :name
     publisher &publishers_contact
     publisher total_publishers_users => 'Number of publisher users'
     isbn :number_10 => 'ISBN-10', :number_13 => 'ISBN-13'
     blurb 'Summary'
...
* Add readme info for using lambda in comma description :
eg:
comma do
...
* Add ability to label passed arrays (still necessary with lambdas improved with labels?)
* Add readme + spec for block with label
eg:

omma :internal do
  bid_creator_name = lambda{ |bid| trade.try(:creator).try(:name) || ''}
  bid_creator_name2 = lambda{ |bid| trade.try(:creator).try(:to_s) || ''}
  reference

  seller :name => 'Seller'
  buyer :name  => 'Buyer'
  bid  bid_creator_name => 'Something'   #ADD INFO ABOUT THIS LINE!
  bid &bid_creator_name2

  grain_type :name
  bin_grade  :code => 'Bin Grade'
  bhc_site   :state, :name => 'Site', :bulk_handler => 'BHC', :port => 'Port Zone'
